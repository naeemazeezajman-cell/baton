...
def _store_bytes(db: Session, user: User, entity: str, entity_id, name: str, data: bytes) -> FileModel:
    ...
    No quota check here by design: this is the sink for server-generated artifacts too
    (reconciliation workbooks, computation PDFs, templates), and a firm at its quota must
    still be able to finish a filing. User uploads are charged in _store_upload."""
...
def _store_upload(db: Session, user: User, entity: str, entity_id, upload: UploadFile) -> FileModel:
    data = read_capped(upload)  # 413 over the per-file cap, before the bytes are held
    check_quota(db, user.tenant_id, len(data))
    return _store_bytes(db, user, entity, entity_id, upload.filename, data)
...
def upload_ledger(fid: uuid.UUID, file: UploadFile, user: User = Depends(current_user),
    ...
    rows = _parse_upload(read_capped(file), LEDGER_COLUMNS, is_ledger=True)
...
def upload_invoices(fid: uuid.UUID, file: UploadFile,
    ...
    rows = _parse_upload(read_capped(file), REGISTER_COLUMNS, is_ledger=False)
    stored = _store_upload(db, user, "vat_filing", f.id, file)
