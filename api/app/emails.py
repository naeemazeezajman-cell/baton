...
EVERY send is attributed to the tenant it originates from: `db` and `tenant_id` are required
keyword arguments all the way down to _send, which drops the message when that tenant is the
demo firm (demo.py). They are required rather than optional-with-a-default on purpose — a new
call site that forgets them raises TypeError in the tests, which is the failure we want. An
optional default would make "forgot to pass the tenant" indistinguishable from "no tenant",
and the demo firm would quietly start emailing real people."""
...
def _send(to: str, subject: str, body: str, *, db, tenant_id,
          reply_to: tuple[str, str] | None = None,
    ...
    Sends from the demo tenant are logged and dropped. They return True — "handled" — so the
    caller's flow completes exactly as it would in a real firm: the proposal still sends, the
    audit trail still records it. The demo is meant to look real; it just cannot reach anyone."""
    if is_demo_tenant(db, tenant_id):
        log.info("[DEMO EMAIL SUPPRESSED] tenant=%s to=%s subject=%r", tenant_id, to, subject)
        return True
    s = get_settings()
    if not s.EMAIL_CONN:
        extra = ""
...
def send_invite(to: str, name: str, firm: str, set_password_link: str,
                temp_password: str | None = None, *, db, tenant_id) -> bool:
    body = (
        f"Hello {name},\n\n"
    ...
    body += "\n— Baton"
    return _send(to, f"You've been invited to {firm} on Baton", body, db=db, tenant_id=tenant_id)


def send_client(to: str, subject: str, body: str,
                reply_to: tuple[str, str] | None = None,
                attachments: list[dict] | None = None, *, db, tenant_id) -> bool:
    ...
    return _send(to, subject, body, db=db, tenant_id=tenant_id,
                 reply_to=reply_to, attachments=attachments)


def send_reset(to: str, name: str, set_password_link: str, *, db, tenant_id) -> bool:
    body = (
        f"Hello {name},\n\n"
    ...
    return _send(to, "Baton password reset", body, db=db, tenant_id=tenant_id)
